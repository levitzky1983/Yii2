...
<div class="row">
    <div class="col-md-4">
        ...
        <?php for ($dayOfMonth = 1; $dayOfMonth <= $calendar->daysInMonth(); $dayOfMonth++): ?>
            <?php if ($calendar->hasDayActivity(date('Y-m-d', mktime(0, 0, 0, $calendar->month, $dayOfMonth, $calendar->year)))): ?>
                <?= \yii\bootstrap\Html::beginTag('td', ['style' => 'background-color:yellow']); ?>
            <?php else: ?>
                <?= \yii\bootstrap\Html::beginTag('td', ['style' => 'background-color:gray']); ?>
            <?php endif; ?>
