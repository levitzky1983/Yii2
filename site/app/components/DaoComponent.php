...
class DaoComponent extends BaseComponent
{
    ...
    public function insertActivity($id_author,$title,$description,$date,$timeBegin,$timeEnd,$isBlocked,$isRepeat,$repeatType,$email){
        ...
        return $this->getConnection()->createCommand($sql,[':id_author'=>$id_author,':title'=>$title,':description'=>$description,':date'=>$date,':timeBegin'=>$timeBegin,':timeEnd'=>$timeEnd,':isBlocked'=>$isBlocked,':isRepeat'=>$isRepeat,':repeatType'=>$repeatType,':email'=>$email])->queryAll();
